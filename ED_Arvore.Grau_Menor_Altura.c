...
void imprimeDados(struct tNo *raiz, int altura, int grauA) {
	
	struct tNo *k;
	
	k = menor (raiz);
	
	printf("\nListando informações da Consulta...\n\n");
	printf("Código: %i\n", raiz->dado);
	printf("Altura: %i\n", altura);
	printf("Grau: %i\n", grauA);
	printf("Menor: %i\n", k);
...
int main(void) {
	...
	int opcao, codigo, q, altura = 0, alturaD = 0, alturaE = 0, valorFB, grauA;
        ...
        switch (opcao) {
        	...
        	case 2: 
				...
				if (p != NULL) {
	        		...
	        		calcAlt(arvore->dir, &alturaD);
					calcAlt(arvore->esq, &alturaE);
					...
					grauA = grau(arvore);
	        		imprimeDados(arvore, altura, grauA); 
				}else
